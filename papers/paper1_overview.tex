...
\begin{enumerate}
  \item An open-source, simulation-validated compiler (\S\ref{sec:compiler})
        from MedGemma weights to MZI phase maps, verified at machine precision.
  ...
  \item A rank sweep (\S\ref{sec:results}) from $r=64$ to full rank across all
        four layer-0 attention projections (Q, K, V, O), with honest reporting
        of SVD approximation error, chip compilation fidelity, and hardware cost.
  \item A chip architecture specification (\S\ref{sec:architecture}) for
        220\,nm SOI silicon photonics with projected power budget.
\end{enumerate}

\noindent
\textbf{Scope.} The current work is a \emph{numerical simulation study}:
we validate the compiler's algorithmic correctness on GPU-extracted weights and
activations.
No optical simulation with physical impairments (loss, crosstalk) or hardware
fabrication is reported.
End-to-end evaluation of downstream medical task accuracy at each rank is
ongoing.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{fig7_compilation_pipeline.pdf}
  \caption{PhotoMedGemma compilation pipeline: from MedGemma weights through
           SVD and Clements decomposition to photonic phase maps and GDS layouts.}
  \label{fig:pipeline}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{fig11_svd_circuit_architecture.pdf}
  \caption{Photonic SVD circuit for one linear layer: $V^\dagger$ Clements mesh,
           $\Sigma$ attenuator bank, $U$ Clements mesh.  Light enters carrying
           $\mathbf{x}$; exits carrying $\mathbf{y} = W\mathbf{x}$.}
  \label{fig:svd_circuit}
\end{figure}

The compilation pipeline (Figure~\ref{fig:pipeline}) proceeds in six stages:
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{fig5_error_comparison_bars.pdf}
  \caption{Three independent error metrics for each layer-0 attention
           projection. SVD approximation error (rank-64) is the dominant
           source; quantisation and compilation errors are negligible.}
  \label{fig:error_bars}
\end{figure}

Three conclusions follow immediately from Table~\ref{tab:comparison} and
Figure~\ref{fig:error_bars}:
\begin{enumerate}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{fig1_svd_error_vs_rank.pdf}
  \caption{SVD approximation error vs.\ rank for all four layer-0 attention
           ...
           ($\sim 10^{-15}$). The v\_proj matrix has the slowest decay.}
  \label{fig:svd_error_rank}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{fig14_svd_error_heatmap.pdf}
  \caption{SVD error heatmap across all projections and ranks.
           v\_proj at rank~64 (0.669) is the hardest case; all projections
           reach zero error at full rank.}
  \label{fig:heatmap}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{fig2_energy_retained_vs_rank.pdf}
  \hfill
  \includegraphics[width=0.48\textwidth]{fig8_error_vs_mzis_tradeoff.pdf}
  \caption{Left: Frobenius-norm energy retained vs.\ rank.
           Right: SVD approximation error vs.\ MZI count
           (lower-left = better).}
  \label{fig:energy_tradeoff}
\end{figure}

The v\_proj matrix has the slowest singular value decay
(Figures~\ref{fig:svd_error_rank}--\ref{fig:heatmap}), indicating that
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{fig10_clements_mesh_schematic.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{fig3_mzi_count_vs_rank.pdf}
  \hfill
  \includegraphics[width=0.48\textwidth]{fig4_chip_area_vs_rank.pdf}
  \caption{Left: MZI count scaling with rank (log-log), with current and
           near-term fabrication limits shown as reference lines.
           Right: Chip area per mesh for Standard and Compact Si technologies.}
  \label{fig:mzi_area_scaling}
\end{figure}
...
$300\,\text{mJ/token}$ for an A100 --- a \textbf{projected 37$\times$
improvement in energy efficiency} (see Section~\ref{sec:discussion} for
caveats).

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{fig6_power_comparison.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{fig13_system_architecture.pdf}
  \caption{System hierarchy: single MZI $\to$ Clements mesh $\to$ 10\,mm chip
           $\to$ multi-chip module $\to$ full MedGemma system.}
  \label{fig:system}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth]{fig9_compilation_fidelity.pdf}
  \hfill
  \includegraphics[width=0.48\textwidth]{fig15_feasibility_zones.pdf}
  \caption{Left: Clements compilation fidelity for $U$ and $V^\dagger$ meshes
           (all at machine precision).
           Right: SVD error vs.\ chip area with manufacturing feasibility
           zones.}
  \label{fig:fidelity_feasibility}
\end{figure}
...
\texttt{output/real/netlist/}, \texttt{output/real/gds/}) targets foundry
submission via multi-project wafer (MPW) runs through ePIXfab or AIM Photonics.
...
Tape-out has not yet been performed.

\paragraph{Power budget caveats.}
The 0\,mW steady-state heater power is idealised: real systems require
thermal trimming ($\approx$0.5--2\,W system-wide) to compensate for
heater-to-heater crosstalk.
Wall-plug laser efficiency ($\sim$10--20\%) increases electrical input
beyond the 800\,mW optical figure.
The $37\times$ efficiency advantage over A100 should be understood as a
projected upper bound under favourable assumptions.
...
\begin{enumerate}
  \item \textbf{No end-to-end medical benchmark evaluation.}
        The SVD errors reported measure layer-level weight reconstruction
        fidelity, not downstream clinical task accuracy on MedQA, PubMedQA,
        or breast cancer classification.
        Whether rank-64 errors (0.19--0.67) degrade diagnostic performance
        meaningfully requires propagating approximate weights through all
        46~layers.
  \item \textbf{No optical simulation.}
        All validation is in exact floating-point arithmetic.
        Real photonic chips will incur propagation loss ($\sim$20--30\,dB for
        a 10\,mm mesh), thermal crosstalk, coupler splitting variations, and
        detector noise.
  \item \textbf{Limited activation samples.}
        Only 5~data points (1~token per image $\times$ 5~images) were captured.
        A thorough evaluation requires diverse clinical inputs and multi-token
        generation.
  \item \textbf{Layer~0 only.}
        Scaling to all 46~layers assumes compilation fidelity and SVD
        compressibility are representative.
  \item \textbf{Vision encoder.}
        The SigLIP-So400M encoder is currently handled electronically; the
        same compilation pipeline applies to its weights but is not yet
        validated.
\end{enumerate}
...
We have presented PhotoMedGemma: a simulation-validated compiler
...
Google DeepMind,
\textit{MedGemma: Medical AI Foundation Models},
