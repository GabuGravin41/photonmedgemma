% ============================================================
% Paper 4: NeurIPS-style conference paper
% "PhotoMedGemma: Machine-Precision Compilation of a
%  Medical Vision-Language Model onto Photonic Hardware"
% ============================================================
% NeurIPS 2026 format (approximate — uses standard article with
% NeurIPS-like layout and conventions)
% ============================================================
...
\author{%
  ...
  \texttt{photomedgemma@github.com}
}
...
% ----------------------------------------------------------------
\begin{abstract}
We present PhotoMedGemma, an open-source compiler that maps the weight matrices
of MedGemma-4B-IT --- a 4-billion-parameter multimodal medical AI --- to phase
settings for silicon photonic Mach-Zehnder interferometer (MZI) meshes.
Photonic chips perform matrix-vector multiplication using light, offering
$\sim 37\times$ better energy efficiency than GPU inference.
...
projections.
We report a complete rank sweep from $r = 64$ to full rank, showing
...
\begin{itemize}
  \item[(1)] A complete open-source compiler from MedGemma weights to
             Clements MZI phase maps, validated at machine precision
  ...
  \item[(3)] A full rank sweep ($r = 64, 128, 256, 512, 1024, 2048$) with
...
\toprule
\textbf{Rank} & \textbf{Energy} & $\varepsilon_{\rm SVD}$ & \textbf{MZIs/mesh} &
...
\url{https://github.com/yourusername/photomedgemma}.
...
Google DeepMind. \textit{MedGemma Technical Report.} 2024.
